...
def cleanup_stale_sessions(max_age_seconds=1800):
    ...
    stale_sessions = [session_id for session_id, session in _sessions.items() if current_time - session['session_last_accessed'] > max_age_seconds]
    for session_id in stale_sessions:
        session = _sessions.pop(session_id, None)
        try:
            import os
            if session and 'dataset_path' in session and os.path.exists(session['dataset_path']):
