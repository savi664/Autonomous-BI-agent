...
async def execute_with_retry(code: str, max_retries=2):
    """
    ...
    for attempt in range(max_retries):
        ...
        if result["status"] == "success":
            ...
            print(
                f"Attempt {attempt + 1} failed with error: {last_error_info['type']} - {last_error_info['message']}"
            )
    ...
    error_message = (
        ...
        if last_error_info
        else "(unknown failure — no error info)"
    )
    return {
        "status": "error",
        "stdout": "",
